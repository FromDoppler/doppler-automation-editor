(function () {
  ...
  function dpEditorDropdown($document, $timeout) {
    var directive = {
      ...
      link: link,
      templateUrl:
        'angularjs/partials/automation/editor/directives/common/dp-dropdown.html',
      scope: {
        arrowSelection: '=',
        keyLabel: '=',
        keyValue: '=',
    ...
    function link(scope, element) {
      ...
      $timeout(function () {
        scope.allowSearch =
          scope.options && scope.options.length > 5 && !scope.arrowSelection;
      });

      scope.onSearchQueryChanged = function () {
        scope.filteredOptions = _.filter(scope.options, function (option) {
          var optionLabel = scope.keyLabel
            ? option[scope.keyLabel]
            : option.label;
      ...
      scope.toggleVisibility = function () {
        if (
          (!scope.disabled && scope.options.length) ||
          (!scope.options.length && scope.expanded)
        ) {
      ...
      function scrollToSelectedItem() {
        ...
        var selectedOptionIndex = _.findIndex(
        ...
        var selectedOptionElement = dropdownListContainer.querySelector(
