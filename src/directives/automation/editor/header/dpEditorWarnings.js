(function() {
  ...
  function dpEditorWarning(warningsStepsService, automation, selectedElementsService, $translate, goToService, COMPONENT_TYPE) {
    var directive = {
      restrict: 'E',
      templateUrl: 'angularjs/partials/automation/editor/directives/header/dp-editor-warnings.html',
      link: link,
      scope: {}
    };
    ...
    function link(scope) {
      scope.componentTypes = ['delay', 'campaign', 'action', 'condition', 'sms', 'push_notification', 'goto', 'initial_condition', 'whatsapp'];
      scope.isOpen = false;
      ...
      scope.selectComponent = function(componentType) {
        var component = automation.getComponentByUid(scope.warningsSteps[componentType][0]);
        if (component) {
      ...
      scope.$watch('warningsSteps', function() {
      ...
      scope.getWarningLabel = function() {
        ...
        if (scope.hasWarningSteps()) {
          ...
          if (count === 1) {
            label = $translate.instant('automation_editor.steps.one_incompleted');
          } else if (count > 1) {
            label = $translate.instant('automation_editor.steps.more_incompleted');
          }
        } else {
      ...
      scope.getComponentWarningLabel = function(componentType) {
        var label;
        if (!scope.warningsSteps[componentType] || !scope.warningsSteps[componentType].length
          || scope.warningsSteps[componentType].length > 1) {
          label = $translate.instant('automation_editor.steps.' + componentType + 's_not_defined');
        } else if (scope.warningsSteps[componentType].length === 1) {
          label = $translate.instant('automation_editor.steps.' + componentType + '_not_defined');
        }
