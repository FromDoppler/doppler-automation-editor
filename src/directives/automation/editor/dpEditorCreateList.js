(function () {
  ...
  angular.module('dopplerApp').directive('dpCreateList', dpCreateList);
  ...
  function dpCreateList(
    ...
    var directive = {
      ...
      link: link,
      templateUrl:
        'angularjs/partials/automation/editor/directives/dp-create-list.html',
    };
    ...
    function link(scope) {
          ...
          if (scope.validationForm.$valid) {
            listService.createList(scope.list).then(function (data) {
              if (data.success) {
                ...
                scope.validationForm['listName'].$setValidity(
                  'createListFailed',
                  data.success
                );
              }
      ...
      scope.getMaxSubscriberLabel = function () {
        return $translate
          .instant('automation_editor.create_list.footer_tip')
          .replace('%1', scope.maxSubsInList);
      };
      ...
      scope.validateSubscriber = function (index) {
        ...
        var duplicateEmail = _.find(
          scope.list.SubscribersList,
          function (subscriber, i) {
            if (i !== index) {
        ...
        scope.validationForm['Email' + index].$setValidity(
          'duplicateEmail',
          !duplicateEmail
        );
