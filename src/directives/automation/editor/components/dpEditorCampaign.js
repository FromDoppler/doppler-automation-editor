(function () {
  ...
  function dpEditorCampaign(
    ...
    var directive = {
      ...
      templateUrl:
        'angularjs/partials/automation/editor/directives/components/dp-editor-campaign.html',
      link: link,
    };
    ...
    function link(scope) {
      ...
      scope.$on('AUTOMATION_SAVED', function () {
        ...
        if (scope.component.thumbnailUrl !== '') {
          $timeout(function () {
            scope.component.thumbnailUrl =
              scope.component.thumbnailUrl.indexOf('?') > 0
                ? scope.component.thumbnailUrl.replace(
                    /([^?]*$)/,
                    Math.random().toString().split('.')[1]
                  )
                : scope.component.thumbnailUrl
                    .concat('?')
                    .concat(Math.random().toString().split('.')[1]);
          }, 1000);
        }
      });
      ...
      scope.openPreview = function (campaign) {
        if (!automation.getIsProcessing()) {
          ...
          automation
            .getHtmlContentForPreview(campaign.id)
      ...
      scope.canBeRemoved = function () {
        scope.showDeleteWarning =
          conditionsDataservice.isEmailLinkedToConditional(
            scope.component.uid
      ...
      scope.isDynamicCampaign = function () {
        return (
          scope.component.campaignType === AUTOMATION_TYPE.ABANDONED_CART ||
