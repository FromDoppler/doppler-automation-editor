(function() {
  ...
  function dpEditorCanvas(automation, AUTOMATION_STATE, CHANGE_TYPE, changesManager, COMPONENT_TYPE,
    selectedElementsService, warningsStepsService, goToService) {
    var directive = {
      restrict: 'E',
      templateUrl: 'angularjs/partials/automation/editor/directives/canvas/dp-editor-canvas.html',
    ...
    function controller($scope) {
      ...
      if ($scope.rootComponent.initialCondition && !$scope.rootComponent.initialCondition.completed) {
        selectedElementsService.setSelectedComponent($scope.rootComponent.initialCondition);
      }

      $scope.$watch(selectedElementsService.getSelectedComponent, function (newComponent, oldComponent) {
        if (!newComponent) {
          goToService.unmarkAllComponentsInGotoSelection(['goto-target-available', 'goto-target-unavailable', 'goto-connected', 'goto-possible-connection', 'dp-tooltip-container']);
        }
        ...
        if (newComponent) {
          ...
          if (newComponent.type === COMPONENT_TYPE.SMS && !newComponent.name) {
            addAutogeneratedSmsName(newComponent);
          }

          if (newComponent.type === COMPONENT_TYPE.WHATSAPP && !newComponent.name) {
            addAutogeneratedWhatsappName(newComponent);
          }
      ...
      function addAutogeneratedWhatsappName(component) {        
        if (component.type === COMPONENT_TYPE.WHATSAPP && !component.name) {
          var oldComponentData;
          var newComponentData;
          if (!automation.getComponentByUid(component.uid)) {
            return;
          }
          var newWhatsappIdName = parseInt($scope.rootComponent.lastWhatsappIdName) || 0;
          newWhatsappIdName = ++newWhatsappIdName;
          var newSmsName = 'WHATSAPP_' + newWhatsappIdName;

          oldComponentData = '';
          newComponentData = newWhatsappIdName;
          component.setData({
            hasUnsavedChanges: true,
            name: newSmsName
          });
          ...
          $scope.rootComponent.lastWhatsappIdName = newWhatsappIdName;
          component.checkCompleted();
          warningsStepsService.checkWarningStep(component);
          automation.checkCompleted();
        }
      ...
      function addAutogeneratedPushName(component) {
        if (component && component.type === COMPONENT_TYPE.PUSH_NOTIFICATION && !component.name) {
          var oldComponentData;
          var newComponentData;
          if (!automation.getComponentByUid(component.uid)) {
            return;
      ...
      function watchSelectedComponentProperties() {
        ...
        propertiesToWatch.forEach(function(property) {
          var watch = $scope.$watch(function() {
            ...
            if ($scope.selectedComponent.type === COMPONENT_TYPE.CAMPAIGN ||
    ...
    function link(scope, element) {
      scope.$on('COMPONENTS_LIST:HIDE', function() {
        var showStepsElements = angular.element(document.querySelector('.add-step-type--container:not(.ng-hide)'));
      ...
      scope.$on('ALTERING_CONDITION_FINISHED', function () {
        setTimeout(function () { goToService.regenerateGotoLinesFromComponentsRegistration(); }, 100);
      });
