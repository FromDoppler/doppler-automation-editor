(function () {
  ...
  function dpEditorImportHtml(
    ...
    var directive = {
      restrict: 'E',
      templateUrl:
        'angularjs/partials/automation/editor/directives/dp-editor-import-html.html',
    ...
    function controller($scope) {
      ...
      $scope.isDynamicAutomation =
        ($scope.rootComponent.automationType === 'abandoned_cart' ||
      ...
      var templateDragAndDropMessage =
        '<span class="changing-message {{dragAndDropClass}}">{{dragAndDropMessage}}</span>';
      $scope.updateUploadedFileStatus = function (
        value,
        fileName,
        errorMessage,
        htmlContent
        ...
        switch (value) {
          ...
          case 1:
            ...
            $scope.updateDragAndDropMessage(
              $translate.instant('automation_editor.import_file.drag_text'),
              ''
            );
            $scope.updateDropButton(
              true,
              $translate.instant('automation_editor.import_file.drag_button')
            );
            break;
          // proccessing
          case 2:
            ...
            $scope.updateDragAndDropMessage(
              $translate.instant('automation_editor.import_file.importing', {
          ...
          case 3:
            ...
            $scope.updateDragAndDropMessage(
              $translate.instant('automation_editor.import_file.imported', {
                fileName: $scope.cropFile(fileName, 50),
              }),
              ''
            );
            $scope.updateDropButton(
              true,
              $translate.instant(
                'automation_editor.import_file.drag_button_imported'
              )
            );
          ...
          case 4:
            ...
            $scope.updateDropButton(
              true,
              $translate.instant(
                'automation_editor.import_file.drag_button_imported_error'
              )
            );
      ...
      $scope.updateDragAndDropMessage = function (
        dragAndDropMessage,
        dragAndDropClass
      ) {
        var miniScope = {
        ...
        $scope.dragAndDropMessage = $interpolate(templateDragAndDropMessage)(
          miniScope
        );
      };
      ...
      $scope.confirmImportedHtml = function () {
        automation
          .saveCampaign(selectedEmail, $scope.htmlContent)
      ...
      if (
        selectedEmail.thumbnailUrl !== '' &&
        selectedEmail.contentType === CONTENT_TYPE.IMPORT
      ) {
        $scope.updateDropButton(
          true,
          $translate.instant(
      ...
      $scope.dropzoneConfig = {
        options: {
          url:
            '/Automation/Content/SubmitFile?idCampaign=' +
            selectedEmail.id.toString() +
          ...
          dictMaxFilesExceeded: $translate.instant(
            'automation_editor.import_file.error_max_files'
          ),
          dictFileTooBig: $translate
            .instant('automation_editor.import_file.error_file_size')
