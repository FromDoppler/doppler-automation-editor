(function () {
  ...
  function dpEditorListsScheduledGrid(
    CHANGE_TYPE,
    changesManager,
    gridService,
    selectedElementsService,
    ...
    var directive = {
      restrict: 'E',
      templateUrl:
        'angularjs/partials/automation/editor/directives/dp-editor-lists-scheduled-grid.html',
    ...
    function controller($scope) {
        ...
        selectedItems: JSON.parse(
          JSON.stringify(
            selectedComponent.operation
              ? selectedComponent.operation.suscriptionLists
              : selectedComponent.suscriptionLists
          )
        ),
      ...
      $scope.listTitle =
        $scope.listTitle ||
        $translate.instant('automation_editor.lists_scheduled_grid.title');
      $scope.listSubtitle =
        $scope.listSubtitle ||
        $translate.instant(
          'automation_editor.lists_scheduled_grid.description'
        );
      ...
      $scope.gridModel = gridService.initGrid({
        ...
        automationId:
          selectedComponent.operation &&
          !automationModel.initialCondition.allSubscribers
            ? automationModel.id
            : 0,
      });
      ...
      $scope.selectRow = function (item) {
        if (
          !document.getElementById('checkbox-' + item.IdSubscribersList)
            .disabled
        ) {
          var result;
          ...
          if (item.IsChecked) {
            ...
            result = _.reject($scope.gridModel.selectedItems, function (data) {
              return (
                data.IdSubscribersList &&
                data.IdSubscribersList === item.IdSubscribersList
              );
            });
      ...
      $scope.IsDisabledRowStatus = function (row) {
        return (
          row.ListStatus == $scope.blockedStatus &&
          !row.IsChecked &&
          !$scope.checkedRows.includes(row.IdSubscribersList)
        );
      };
    }
  }
