(function () {
  ...
  function dpEditorFooter(
    ...
    var directive = {
      restrict: 'E',
      templateUrl:
        'angularjs/partials/automation/editor/directives/footer/dp-editor-footer.html',
      link: link,
    };
    ...
    function link(scope) {
      ...
      scope.confirmListSelection = function () {
        ...
        $rootScope.$broadcast(
          selectedComponent.type !== COMPONENT_TYPE.CONDITION
          ...
          automation.hasDynamicElement().then(function (result) {
            if (result) {
              ...
              ModalService.showModal({
                templateUrl:
                  'angularjs/partials/automation/noDynamicElementModal.html',
      ...
      scope.stopCampaign = function () {
        ModalService.showModal({
          templateUrl:
            '/angularjs/partials/automation/stopAutomationModal.html',
          ...
          modal.close.then(function (response) {
            if (response) {
              ...
              automation.stopAutomationCampaign().then(
                function (res) {
      ...
      scope.showContentInformation = function () {
        return ModalService.showModal({
          templateUrl:
            '/angularjs/partials/automation/completeInformation.html',
      ...
      scope.getAutomationTipLabel = function () {
        ...
        if (
          isFlowComplete === AUTOMATION_COMPLETED_STATE.WITH_DELETED_FIELDS ||
          isFlowComplete ===
          ...
          label = $translate.instant(
            'automation_editor.footer.tip_state_error'
          );
        } else if (scope.hasNotSmsCredits()) {
          label = $translate.instant(
            'automation_editor.footer.tip_not_sms_credit'
          );
        } else {
          label = $translate.instant(
            'automation_editor.footer.tip_state_' + isFlowComplete
          );
        }
        return label;
      };
      ...
      scope.getAutomationBtnLabel = function () {
        ...
        if (scope.rootComponent.state !== AUTOMATION_STATE.ACTIVE) {
          if (
            ...
            label = $translate.instant(
              'automation_editor.buttons.start_campaign_warning'
            );
          } else if (scope.rootComponent.state === AUTOMATION_STATE.PAUSED) {
            label = $translate.instant(
              'automation_editor.buttons.restart_campaign'
            );
          } else if (scope.rootComponent.state === AUTOMATION_STATE.STOPPED) {
            label = $translate.instant(
              'automation_editor.buttons.reactivate_campaign'
            );
          } else {
            label = $translate.instant(
              'automation_editor.buttons.start_campaign'
            );
          }
          return label;
        }
      ...
      scope.hasErrors = function () {
        return (
          ...
          scope.isFlowComplete() ===
            AUTOMATION_COMPLETED_STATE.WITH_TRIAL_DISABLED ||
          scope.isFlowComplete() ===
            AUTOMATION_COMPLETED_STATE.WITH_NON_INTEGRATION ||
          scope.isFlowComplete() ===
            AUTOMATION_COMPLETED_STATE.WITH_TRIAL_EXPIRED
        );
      };
      ...
      scope.pauseCampaign = function () {
        ModalService.showModal({
          templateUrl:
            '/angularjs/partials/automation/pauseAutomationModal.html',
          controller: 'ModalYesOrNoCtrl',
          inputs: { data: {} },
        }).then(function (modal) {
          modal.close.then(function (response) {
      ...
      function showPausedConfirmationModal() {
        ModalService.showModal({
          templateUrl:
            '/angularjs/partials/automation/pauseAutomationMessageModal.html',
          controller: 'ModalYesOrNoCtrl',
          inputs: { data: {} },
        });
      }
