(function () {
  ...
  angular
    ...
    .directive('dpEditorPrivateDomainValidation', [
      'domainDataService',
      dpEditorPrivateDomainValidation,
    ]);
  ...
  function dpEditorPrivateDomainValidation(domainDataService) {
    ...
    function link($scope, element, attr, ctrl) {
      ...
      $scope.validatePrivateDomain = function (fromEmail) {
        ...
        $scope.confirmedDomain =
          $scope.$parent.selectedComponent.confirmedDomain;
        fromEmail = fromEmail || '';
        var domain = '';
        ...
        if (fromEmail.length > 0) {
          ...
          if (domain !== null && domain.length > 0) {
            ...
            if (
              !(
                !!$scope.confirmedDomain &&
                domain.toLowerCase() === $scope.confirmedDomain.toLowerCase()
              )
            ) {
              domainDataService.isValidDomain(domain).then(function (response) {
                if (response.data.success && response.data.valid) {
                  ...
                  $scope.$parent.campaignForm.fromEmail.$setValidity(
                    'privatedomain',
                    true
                  );
