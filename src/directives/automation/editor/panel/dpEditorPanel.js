  ...
  function dpEditorPanel(automation, selectedElementsService, AUTOMATION_TYPE, COMPONENT_TYPE, CONDITION_TYPE, $compile) {
    ...
    function controller($scope, $element) {
      ...
      $scope.$watch(selectedElementsService.getSelectedComponent, function (newComponent) {
        ...
        var panelElement = angular.element($element[0].querySelector('.component-panel-container'));
        var template = newComponent? newComponent.panelTemplate : '<div></div>';
    ...
    function link(scope) {
      ...
      scope.hasSelectedElement = function() {
        return scope.selectedComponent && (scope.selectedComponent.type === COMPONENT_TYPE.DELAY ||
