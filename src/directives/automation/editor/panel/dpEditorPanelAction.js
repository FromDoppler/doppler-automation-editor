(function() {
  ...
  function dpEditorPanelAction($rootScope, $translate, ACTION_TYPE, automation, AUTOMATION_TYPE, COMPONENT_TYPE,
    LIST_SELECTION_STATE, optionsListDataservice, selectedElementsService,
    userFieldsDataservice, FIELD_TYPE, settingsService, utils) {

    var directive = {
      restrict: 'AE',
      templateUrl: 'angularjs/partials/automation/editor/directives/panel/dp-editor-panel-action.html',
      link: link
    };
    ...
    function link(scope) {
      var listTitle = $translate.instant('automation_editor.components.action.list_grid.title');
      var listSubtitle = $translate.instant('automation_editor.components.action.list_grid.subtitle');
      ...
      scope.$watch(selectedElementsService.getSelectedComponent, function(newSelectedComponent) {
        if (newSelectedComponent && newSelectedComponent.type === COMPONENT_TYPE.ACTION) {
          if (newSelectedComponent.operation
            && newSelectedComponent.operation.field
            && newSelectedComponent.operation.field.type === FIELD_TYPE.DATE) {
            scope.datePickerPopup.value = moment(newSelectedComponent.operation.field.date).toDate();
          }
      ...
      scope.$watch('datePickerPopup.visible', function(newValue, oldValue) {
        if (newValue !== oldValue && !newValue) {
          if (scope.datePickerPopup.value) {
            scope.selectedComponent.operation.field.date = scope.datePickerPopup.value.toISOString().substring(0, 10);
          }
          scope.datePickerPopup.value = moment(scope.selectedComponent.operation.field.date).toDate();
        }
      });

      scope.$watch('scope.selectedComponent.operation.field.date', function(newValue) {
        if (newValue) {
          scope.datePickerPopup.value = moment(scope.selectedComponent.operation.field.date).toDate();
        }
      });
      ...
      scope.showListSelectionSimple = function() {
        scope.toggleListSelection(LIST_SELECTION_STATE.SIMPLE, listTitle, listSubtitle);
      };

      scope.showListSelectionMultiple = function() {
        var listTitle = $translate.instant('automation_editor.components.action.remove_grid.title');
        var listSubtitle = $translate.instant('automation_editor.components.action.remove_grid.subtitle');
        scope.toggleListSelection(LIST_SELECTION_STATE.MULTIPLE, listTitle, listSubtitle);
      };
      ...
      scope.onActionSelected = function(rawActionData, oldAction) {
        ...
        scope.selectedComponent.operation = automation.createOperation(getDefaultOperationData(rawActionData));
      };

      scope.onEmailSelected = function(newEmailData, oldEmail) {
        if (oldEmail && oldEmail.idEmail !== newEmailData.idEmail || !oldEmail) {
      ...
      scope.showErrorFor = function(propertyElement) {
        ...
        case 'subscriptionList':
          if (scope.selectedComponent && scope.selectedComponent.touched
          && (scope.selectedComponent.operation === ACTION_TYPE.ASSOCIATE_SUBSCRIBER_TO_LIST
        ...
        case 'action_emails':
          if (scope.emailComponentsToBind && !scope.emailComponentsToBind.length) {
            showError = true;
          }
      ...
      scope.onSubscriberFieldSelected = function(rawFieldData, oldField) {
        ...
        scope.selectedComponent.operation.field = Object.assign({}, rawFieldData);
