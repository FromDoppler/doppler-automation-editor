(function () {
  ...
  function dpEditorPanelCampaign(
    $rootScope,
    $translate,
    automation,
    AUTOMATION_TYPE,
    ...
    var directive = {
      restrict: 'AE',
      templateUrl:
        'angularjs/partials/automation/editor/directives/panel/dp-editor-panel-campaign.html',
      link: link,
    };
    ...
    function link(scope) {
      ...
      scope.listToSend = [
        {
          ...
          label: $translate.instant(
            'automation_editor.sidebar.campaign_send_test_subscribers_select_placeholder'
          ),
          isBlocked: false,
        },
      ];
      ...
      settingsService.getSettings().then(function (response) {
        ...
        if (response.domainKeyList.length) {
          ...
          scope.selectedComponent.DomainKeyLabel =
          ...
          if (scope.selectedComponent.fromEmail.length) {
            ...
            scope.domainKey = fromEmail.substring(
              fromEmail.indexOf('@') + 1,
              fromEmail.length
            );
            scope.selectedComponent.dkimEmail = fromEmail.substring(
              0,
              fromEmail.indexOf('@')
            );
            ...
            if (domainKeyData) {
              ...
              scope.selectedComponent.idDomainKeySelected =
                domainKeyData.DomainId;
            } else {
              scope.selectedComponent.fromEmail =
                scope.selectedComponent.dkimEmail +
                '@' +
                scope.selectedComponent.DomainKeyLabel;
          ...
          scope.$watch(
            'selectedComponent.idDomainKeySelected',
            setDkimLabelAndUpdateFromEmail
          );
        ...
        scope.isIgnoreContactPolicySetupCompleted =
          response.isIgnoreContactPolicySetupCompleted;
      ...
      function setDkimLabelAndUpdateFromEmail(domainKeyId) {
        if (!!domainKeyId) {
          ...
          scope.selectedComponent.DomainKeyLabel = existingDomainKey
            ? existingDomainKey.DomainName
            : scope.domainKeyList[0].DomainName;
      ...
      scope.MarkDMARCDomainWarningDisplayed = function () {
        var domain =
          scope.campaignForm.fromEmail.$viewValue.match('[^@@]+$')[0];
        scope.selectedComponent.DMARCAcceptedDomain = domain
          .split('.')[0]
          .toLowerCase();
      ...
      scope.includedInDmarcDomains = function (domain) {
        return (
          scope.dmarcDomains &&
          scope.dmarcDomains.includes(domain.toUpperCase().trim())
        );
      };
      ...
      scope.toggleTemplates = function (value) {
        if (value && changesManager.getUnsavedChanges()) {
          ...
          automation.saveChanges().then(
            function () {
              $rootScope.$broadcast('UPDATE_SAVING_STATE');
              automation.setIsProcessing(false);
              if (scope.selectedComponent.thumbnailUrl === '') {
                scope.toggleTemplateView(
                  true,
                  scope.selectedComponent.campaignType
                );
              } else {
                window.location.href = templatesService.getEditorCampaignUrl(
                  scope.selectedComponent.id,
                  scope.selectedComponent.editorType
                );
              }
            },
            function () {
      ...
      scope.sendTestList = function () {
        ...
        automation.saveChanges().then(function () {
          ...
          automation
            .sendTestToList(
              scope.selectedComponent.idList,
              scope.selectedComponent.id
            )
      ...
      scope.sendTestToEmails = function () {
        ...
        automation.saveChanges().then(function () {
          ...
          automation
            .sendTestToEmails(
              scope.selectedComponent.emailList,
              scope.selectedComponent.id
            )
            .then(function (response) {
              scope.sendingTest = false;
      ...
      scope.inputKeyup = function (event, key) {
        var unprocessableKeys = [
          8, 9, 13, 16, 17, 18, 20, 27, 37, 38, 39, 40, 91, 93,
        ];
      ...
      scope.emailNameOnBlur = function () {
        ...
        if (!currentEmailName) {
          scope.selectedComponent.name =
            'Email_' + ++scope.rootComponent.lastEmailIdName;
        }
      };
      ...
      function onEmailNameChange(newValue, oldValue) {
        ...
        if (
          ...
          scope.selectedComponent.uid !== selectedCampaignUid
        ) {
      ...
      scope.setChanged = function () {
        var rssElement = angular.element(
          document.querySelector('input[name="rss"]')
        );
        ...
        automation.validateAndAutoCompleteRSS(rssElement.val()).then(
          function (response) {
      ...
      function updateRssValue(newValue) {
        angular
          .element(document.querySelector('input[name="rss"]'))
          .val(newValue);
        scope.campaignForm.rss.$setValidity('rss', true);
      }
      ...
      function updateDkimEmail(newFromEmail) {
        if (!!newFromEmail) {
          // eslint-disable-line no-extra-boolean-cast
          scope.selectedComponent.dkimEmail =
            newFromEmail.indexOf('@') !== -1
              ? newFromEmail.substring(0, newFromEmail.indexOf('@'))
              : '';
        }
      }
      ...
      scope.updateFromEmailDkim = function (newDkimEmail) {
        var newFromEmail =
          !newDkimEmail || !newDkimEmail.length
            ? ''
            : newDkimEmail + '@' + scope.selectedComponent.DomainKeyLabel;
      ...
      scope.setNewRssValue = function (value) {
        ...
        var oldComponentData = JSON.parse(
          JSON.stringify(selectedComponent.rss)
        );
      ...
      scope.isDynamicContent = function () {
        return (
          scope.automationType === AUTOMATION_TYPE.ABANDONED_CART ||
      ...
      dopplerScripts.emojiSubject.addons({
        rootElement: emojiButtonElement.closest(
          '.dp-editor-panel-campaign-component'
        ),
        buttonElement: emojiButtonElement,
        inputElement: document.getElementById('subject_property'),
        categoriesTranslation: {
          categories: {
            smileys: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Faces_Emotions'
            ),
            people: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_People_Body'
            ),
            animals: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Animals_Nature'
            ),
            food: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Food_Drinks'
            ),
            activities: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Activities'
            ),
            travel: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Travel'
            ),
            objects: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Objects'
            ),
            symbols: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Symbols'
            ),
            flags: $translate.instant(
              'automation_editor.sidebar.campaign_subject_emojis.emojis_categories.Emoji_Category_Flags'
            ),
          },
        },
      });
