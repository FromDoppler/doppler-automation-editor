(function () {
  ...
  function dpEditorPanelCondition(
    $rootScope,
    $timeout,
    $translate,
    automation,
    ...
    var directive = {
      restrict: 'AE',
      templateUrl:
        'angularjs/partials/automation/editor/directives/panel/dp-editor-panel-condition.html',
      link: link,
    };
    ...
    function link(scope) {
      ...
      scope.conditionPlaceholder = $translate.instant(
        'automation_editor.components.condition.conditionals.placeholder'
      );
      scope.comparisonOperators =
        optionsListDataservice.getComparisonOperators();
      ...
      scope.campaignBehaviorEvents =
        optionsListDataservice.getCampaignBehaviorEvents();
      scope.genderFieldOptions = optionsListDataservice.getGenderFieldOptions();
      scope.listMembershipEvents =
        optionsListDataservice.getListMembershipEvents();
      ...
      scope.setSelectedConditional =
        selectedElementsService.setSelectedConditional;
      ...
      scope.abandonedCartInformationDefaultFields =
        optionsListDataservice.getAbandonedCartInformationDefaultFields();
      ...
      scope.$watch(
        ...
        function (newSelectedComponent) {
          ...
          if (
            ...
            newSelectedComponent.type === COMPONENT_TYPE.CONDITION
          ) {
            loadEmailsAndLinks();
          }
      ...
      scope.$watch(
        selectedElementsService.getSelectedConditional,
        function (newConditional, oldConditional) {
      ...
      scope.$watch('datePickerPopup.visible', function (newValue, oldValue) {
        if (newValue !== oldValue && !newValue) {
          if (scope.datePickerPopup.value) {
            scope.selectedConditional.field.date = scope.datePickerPopup.value
              .toISOString()
              .substring(0, 10);
          }
          scope.datePickerPopup.value = moment(
            scope.selectedConditional.field.date
          ).toDate();
        }
      });
      ...
      function createConditionalWatches() {
        var propertiesToWatch =
          scope.selectedConditional.getPropertiesToWatch();
          ...
          var watch = scope.$watch(
            ...
            function (newValue, oldValue) {
              ...
              changesManager.add({
                ...
                conditionalIndex: scope.selectedComponent.getConditionalIndex(
                  scope.selectedConditional
                ),
                key: property,
              ...
              conditionsDataservice.updateDuplicatesOfConditional(
                scope.selectedConditional.uid
              );
              checkFlowStatus();
              //check if the selected conditional is a duplicate of another conditional
              conditionsDataservice.checkDuplicateConditionals(
                scope.selectedConditional.uid
              );
            },
            true
      ...
      scope.onConditionalSelected = function (
        rawConditionalData,
        oldConditional,
        index
      ) {
        ...
        if (
          oldConditional &&
          oldConditional.type === rawConditionalData.value
        ) {
          return;
        }
        scope.toggleAddConditionalDropdown(false);
        newConditional = automation.createConditional(
          getDefaultConditionalData(rawConditionalData)
        );
        scope.selectedComponent.addConditional(
          newConditional,
          oldConditional,
          index
        );
        ...
        newIndex =
          index >= 0 ? index : scope.selectedComponent.conditionals.length - 1;
        selectedElementsService.setSelectedConditional(
          scope.selectedComponent.conditionals[newIndex]
        );
        checkFlowStatus();
      };
      ...
      scope.formatDate = function (date) {
        return (
          '&quot;' + moment(date).format(scope.format.toUpperCase()) + '&quot;'
        );
      };
      ...
      scope.showListSelectionSimple = function () {
        var title = $translate.instant(
          'automation_editor.components.condition.conditionals.list_membership.list_selection.title'
        );
        var subtitle = $translate.instant(
          'automation_editor.components.condition.conditionals.list_membership.list_selection.subtitle'
        );
      ...
      scope.showVerificationTimeForm = function () {
        return !!(
          scope.selectedComponent &&
          _.find(scope.selectedComponent.conditionals, function (conditional) {
            return (
      ...
      scope.showErrorFor = function (propertyElement) {
        ...
        switch (propertyElement) {
          ...
          case 'campaign_behavior_events':
            if (
              (scope.selectedConditional.event ===
                CONDITIONAL_EVENT.ANY_LINK_CLICKED ||
                scope.selectedConditional.event ===
              ...
              (scope.selectedConditional.event !==
                CONDITIONAL_EVENT.LINK_CLICKED ||
                scope.selectedConditional.event !==
          ...
          case 'subscriber_information_fields':
            if (
              !scope.selectedConditional.field ||
              scope.selectedConditional.field.deleted
            ) {
              showError = true;
            }
            break;
          ...
          case 'subscriber_information_value':
            if (
              scope.selectedConditional.field.value &&
              !scope.selectedConditional.field.value.length
            ) {
              showError = true;
            } else if (
              scope.selectedConditional.duplicate !== DUPLICATE_STATE.ORIGIN &&
      ...
      function resetLinksForSelectedEmail() {
        ...
        if (
          !scope.selectedConditional.email ||
          scope.selectedConditional.email.uidEmail === 0 ||
          scope.selectedConditional.type !== CONDITIONAL_TYPE.CAMPAIGN_BEHAVIOR
        ) {
          return;
        }
        conditionalsWithLinks = _.filter(
          scope.selectedComponent.conditionals,
          function (conditional) {
            return (
              conditional.type === CONDITIONAL_TYPE.CAMPAIGN_BEHAVIOR &&
              (conditional.event === CONDITIONAL_EVENT.LINK_CLICKED ||
                conditional.event === CONDITIONAL_EVENT.LINK_NOT_CLICKED) &&
              conditional.link.idLink !== 0 &&
              conditional.uid !== scope.selectedConditional.uid
            );
          }
        );
        ...
        if (linksInUse.length) {
          _.each(linksInUse, function (linkInUse) {
            scope.selectedEmailLinks = _.without(
              scope.selectedEmailLinks,
      ...
      function filterCampaignBehaviorEvents() {
        ...
        if (
          ...
          if (scope.selectedConditional.email) {
            automation
              .hasDynamicElement(scope.selectedConditional.email.idEmail)
              .then(function (result) {
                if (!result) {
                  scope.campaignBehaviorEvents = _.filter(
                    scope.campaignBehaviorEvents,
                    function (event) {
                      return (
                        event.value !==
                          CONDITIONAL_EVENT.ANY_DYNAMIC_LINK_CLICKED &&
                        event.value !==
                          CONDITIONAL_EVENT.NO_DYNAMIC_LINK_CLICKED
                      );
                    }
                  );
                }
          ...
          scope.campaignBehaviorEvents = _.filter(
            ...
            function (event) {
              return (
                event.value !== CONDITIONAL_EVENT.ANY_DYNAMIC_LINK_CLICKED &&
      ...
      scope.validateUrlDomain = function (conditional, index) {
        ...
        if (
          ...
          scope.selectedConditional.siteBehaviorForm[
            'url' + index
          ].$setValidity('domain', false);
          return false;
        }
      ...
      scope.onVisitedPageSelected = function (value, domain, field) {
        ...
        if (field === 'visitedPage') {
          scope.selectedConditional.event = value
            ? CONDITIONAL_EVENT.VISITED
