(function () {
  ...
  function dpEditorPanelGoto(
    ...
    var directive = {
      restrict: 'AE',
      templateUrl:
        'angularjs/partials/automation/editor/directives/panel/dp-editor-panel-goto.html',
      link: link,
    };
    ...
    function link(scope) {
      ...
      idsToMarkInSelection.push(
        scope.selectedComponent && scope.selectedComponent.uid
      );
      if (scope.selectedComponent && scope.selectedComponent.line) {
        scope.selectedComponent.line = goToService.applyGotoSelectionStyle(
          scope.selectedComponent.line
        );
        idsToMarkInSelection.push(scope.selectedComponent.goto);
      }
      goToService.markComponentsInGotoSelection(idsToMarkInSelection, true, [
        'goto-connected',
      ]);
      ...
      scope.gotoSelected = function (value) {
        if (value !== scope.selectedComponent.goto) {
          ...
          scope.selectedComponent.line =
            goToService.drawGoToLineBetweenComponents({
          ...
          goToService.markComponentInGotoSelection(value, true, [
            'goto-connected',
          ]);
        }
      ...
      scope.getDropDownLabel = function () {
        return scope.selectedComponent && scope.selectedComponent.goto
          ? getUidComponentLabel(scope.selectedComponent.goto)
          : $translate.instant(
              'automation_editor.canvas.goto_step_placeholder'
            );
      };

      function addGoToSelectionIndexToComponent(
        componentUid,
        goToSelectionIndex
      ) {
        document
          .querySelector('#component_'.concat(componentUid))
          .getElementsByClassName('uid-label-component')[0].innerText =
          goToSelectionIndex;
      }

      function getComponentList() {
        var componentList = [];
        var availablesComponentList =
          scope.selectedComponent.gotoComponentsAvailables;
        for (var i = 0; i < availablesComponentList.length; i++) {
          ...
          addGoToSelectionIndexToComponent(
            availablesComponentList[i].component.uid,
            availablesComponentList[i].goToSelectionIndex
          );
        }
      ...
      function getLabelComponent(item) {
        var imagePath = '../images/automation-'
          .concat(item.component.type)
          .concat('.svg');
        var imgTag = "<img class='dropdown-image' src='"
          .concat(imagePath)
          .concat("'/>");
        var spanUidTag = "<span class='dropdown-uid-label'>"
          .concat(item.goToSelectionIndex)
          .concat('.</span>');
        var componentName =
          item.component.name && item.component.name.length > 0
            ? item.component.name
            : $translate.instant(
                'automation_editor.canvas.'
                  .concat(item.component.type)
                  .concat('_icon')
              );
        var spanNameTag = "<span class='dropdown-name-label'>"
      ...
      function setComponentAvailability(components, from, index) {
        ...
        if (component.type == 'condition') {
          //if both branch has available components the condition is a available component too
          var positive = setComponentAvailability(
            component.positiveSiblings,
            from
          );
          var negative = setComponentAvailability(
            component.negativeSiblings,
            from
          );
          ...
          //if the next is an available component, this is an available component too
          value = setComponentAvailability(components, from, index + 1);
        }
        if (value && component.type !== COMPONENT_TYPE.GOTO_STEP) {
          goToSelectionIndex++;
          scope.selectedComponent.gotoComponentsAvailables.push({
            component: component,
            goToSelectionIndex: goToSelectionIndex,
          });
        } else {
      ...
      function isGotoComponentDefined(component) {
        return (
          component.type === COMPONENT_TYPE.GOTO_STEP && component.goto > 0
        );
      }
      ...
      function markComponentsUnavailables() {
        ...
        document
          .querySelector('.initial-step--container')
          .classList.add('goto-target-unavailable');
        var unavailablesComponentList = gotoComponentsUnavailables;
        for (var i = 0; i < unavailablesComponentList.length; i++) {
          goToService.markComponentInGotoSelection(
            unavailablesComponentList[i].uid,
            true,
            ['goto-target-unavailable', 'dp-tooltip-container']
          );
        }
        goToService.markComponentInGotoSelection(
          scope.selectedComponent.uid,
