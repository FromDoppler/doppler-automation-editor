(function () {
  ...
  angular.module('dopplerApp').directive('dpTemplates', dpTemplates);
  ...
  function dpTemplates(
    ...
    function controller($scope) {
      $scope.categoryFilter = $location.search().category
        ? $location.search().category.toLowerCase()
        : '';
      ...
      $scope.isPublicTemplatesActive =
        $scope.allTemplates || !!$scope.categoryFilter;
      $scope.dateFormat =
      ...
      $scope.inAutomationMode = function () {
        return $scope.allTemplates;
      };
      $scope.inTemplatesSectionMode = function () {
        return !$scope.allTemplates;
      };
      $scope.inTemplateCreationMode = function () {
        return (
          $scope.inTemplatesSectionMode() && $scope.isPublicTemplatesActive
        );
      };
      $scope.inTemplateChoosingMode = function () {
        return (
          $scope.inTemplatesSectionMode() && !$scope.isPublicTemplatesActive
        );
      };
      ...
      function getCategoriesPublicTemplates() {
        ...
        $translate.onReady().then(function () {
          allCategories = {
            IdTemplateCategory: 0,
            Name: $translate.instant('all_categories'),
          };
        });

        $scope.categories = [];
        templatesService.getCategoriesPublicTemplates().then(function (result) {
          var resultOrderByName = dopplerScripts.utils.orderByCriteria(
            result,
            'Name'
          );
          result = dopplerScripts.utils.moveItemToFirstPosition(
            resultOrderByName,
            2,
            'IdTemplateCategory'
          );
          $scope.categories = dopplerScripts.utils.moveItemToFirstPosition(
            result,
            1,
            'IdTemplateCategory'
          );
          if ($scope.allTemplates) {
            $scope.categories.push({
              IdTemplateCategory: -2,
              Name: $translate.instant('templates_title'),
            });
          }
          $scope.categories.push(allCategories);
          $scope.categorySelected = {
            IdTemplateCategory: $scope.categories[0].IdTemplateCategory,
          };
          if ($scope.categoryFilter) {
            var category = _.find(result, function (cat) {
              return (
                cat.NameToFilter && cat.NameToFilter === $scope.categoryFilter
              );
            });
            if (category) {
              $scope.categorySelected = {
                IdTemplateCategory: category.IdTemplateCategory,
              };
            }
          }
      ...
      $scope.renameTemplate = function (template) {
        ModalService.showModal({
          ...
          inputs: {
            data: {
              title: $translate.instant('templates_rename_popup.title', {
                name: template.Name,
              }),
              description: $translate.instant(
                'templates_rename_popup.description'
              ),
              buttonCancelLabel: $translate.instant('button_cancel'),
              buttonPrimaryLabel: $translate.instant(
                'templates_rename_popup.button_primary'
              ),
          ...
          modal.close.then(function (response) {
            if (response.isConfirmed && response.name) {
              templatesService
                .renameTemplate(template.IdTemplate, response.name)
                .then(function () {
                  resetTemplates();
                  getTemplates(0, true);
                });
      ...
      $scope.duplicateTemplate = function (template) {
        var fieldValue = $translate.instant('templates_duplicate_popup.name', {
          name: template.Name,
        });
        ...
        ModalService.showModal({
          ...
          inputs: {
            data: {
              title: $translate.instant('templates_duplicate_popup.title', {
                name: template.Name,
              }),
              description: $translate.instant(
                'templates_duplicate_popup.description'
              ),
              buttonCancelLabel: $translate.instant('button_cancel'),
              buttonPrimaryLabel: $translate.instant(
                'templates_duplicate_popup.button_primary'
              ),
              buttonPrimaryClass: 'button--primary button--small',
          ...
          modal.close.then(function (response) {
            if (response.isConfirmed && response.name) {
              templatesService
                .duplicateTemplate(template.IdTemplate, response.name)
                .then(function () {
                  resetTemplates();
                  getTemplates(0, true);
                });
      ...
      $scope.deleteTemplate = function (template) {
        ModalService.showModal({
          ...
          inputs: {
            data: {
              title: $translate.instant('templates_delete_popup.title', {
                name: template.Name,
              }),
              description: $translate.instant(
                'templates_delete_popup.description'
              ),
              buttonCancelLabel: $translate.instant('button_cancel'),
              buttonPrimaryLabel: $translate.instant(
                'templates_delete_popup.button_primary'
              ),
      ...
      $scope.getCreateCampaignFromTemplateUrl = function (id, editorType) {
        return templatesService.getCampaignCreationFromTemplateUrl(
          id,
          editorType
        );
      };
      ...
      $scope.selectTemplate = function (id, editorType) {
        if (!$scope.selectedTemplateForCreate) {
          ...
          templatesService.createTemplateFromPublicAndRedirect(
            id,
            editorType,
            function (error) {
              console.error(
                'Error on createTemplateFromPublicAndRedirect',
                error
              );
              $scope.selectedTemplateForCreate = false;
            }
      ...
      $scope.onScroll = function () {
        ...
        if ($scope.isPublicTemplatesActive) {
          return getTemplates(
            $scope.categorySelected
              ? $scope.categorySelected.IdTemplateCategory
              : 0,
            false
          );
        }
      ...
      $scope.useTemplateAsIs = function (template) {
        ...
        setLocationWithIdCampaign(false, function () {
          if (
            !templatesService.tryToRedirectToSetCampaignFromTemplateIfUnlayerAndContinue(
              template.IdTemplate,
              template.EditorType,
            ...
            automation
              .saveTemplateContent(selectedComponent.id, template.IdTemplate)
              .then(function () {
      ...
      $scope.newAutomationCampaign = function (template) {
        ...
        setLocationWithIdCampaign(false, function () {
          if (
            !templatesService.tryToRedirectToSetCampaignFromTemplateIfUnlayerAndEdit(
              template.IdTemplate,
              template.EditorType,
              selectedComponent.id
            )
      ...
      function setCampaignThumbnail(selectedComponent) {
        var data = {
          ...
          thumbnailUrl:
            $scope.settings.urlBase +
            '/Users/' +
            $scope.settings.idUser +
            '/Campaigns/' +
            selectedComponent.id +
            '/' +
            selectedComponent.id +
