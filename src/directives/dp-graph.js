(function () {
  ...
  angular.module('dopplerApp').directive('dpgraph', dpgraph);
  ...
  function dpgraph() {
    ...
    function link(scope) {
      if (scope.type === 'c3' && scope.c3config && scope.c3config.data) {
        if (
          scope.c3config.data.type !== undefined &&
          scope.c3config.data.type === 'donut'
        ) {
        ...
        scope.$watchCollection('c3config', function () {
          ...
          if (
            scope.c3config.data.type !== undefined &&
            scope.c3config.data.type !== 'donut' &&
            scope.c3config.legend.show === false
          ) {
        ...
        scope.generateCustomLegend = function () {
          ...
          d3.select('.' + id.substr(1))
            .insert('div', '.chart')
            .attr('class', ' legend')
            .selectAll('span') //eslint-disable-line no-undef
            .data(ids)
            .enter()
            .append('span')
          ...
          $(id + ' .legend-container').each(function (index, item) {
            //eslint-disable-line no-undef
            if ($(item).text().length > 28) {
              //eslint-disable-line no-undef
              $(item).html(
                "<span class='titip-top titip-fade' data-title='" +
                  $(item).text() +
                  "'><span class='legend-icon' style='" +
                  $(item).attr('style') +
        ...
        scope.$watchCollection('c3config', function () {
          scope.totalFunnel =
            scope.c3config[0].value === 0 ? 1 : scope.c3config[0].value;
