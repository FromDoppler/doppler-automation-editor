(function () {
  ...
  smsSettingsService.$inject = ['$http', 'PAYMENT_METHOD'];
  ...
  function smsSettingsService($http, PAYMENT_METHOD) {
    ...
    function getUserSmsData() {
      return $http
        .get('/ControlPanel/AccountPreferences/GetUserSmsData')
        .then(function (response) {
          return {
            status: response.status,
            data: {
              enabledCountries: _.map(
              ...
              creditCardTypes: _.map(
                response.data.creditCardTypes,
                function (cardType) {
                  return {
    ...
    function saveSelectedSmsCountries(regionCodes) {
      return $http
        .post(
          '/ControlPanel/AccountPreferences/SaveSelectedSmsCountries',
          regionCodes
        )
        .then(function (response) {
    ...
    function saveDeliverySmsConfig(deliveryConfig) {
      return $http
        .post(
          '/ControlPanel/AccountPreferences/SaveSmsDeliverySettings',
          deliveryConfig
        )
        .then(function (response) {
          return response.data;
    ...
    function buySmsCredits(billingInformation, fee, paymentMethod) {
      var billingInformationModel = mapBillingInformationModel(
        billingInformation,
        fee,
        paymentMethod
      );
      return $http
        .post(
          '/ControlPanel/AccountPreferences/BuySmsCredits',
    ...
    function mapBillingInformationModel(
      billingInformation,
      fee,
      paymentMethod
    ) {
