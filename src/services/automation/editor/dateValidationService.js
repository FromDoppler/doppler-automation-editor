(function () {
  ...
  function dateValidation(
      ...
      if (frequency) {
        getFullDateinLocalTime(
          ...
          if (duration.asMinutes() > 0 || Math.abs(duration.asMinutes()) < 15) {
            var momentFullDate = moment(fullDate).add(
              duration.asHours(),
              'hours'
            );
            var roundedCurrentMinutes = Math.ceil(
              Math.round(moment(momentFullDate).minute() / 15) * 15
            );
            ...
            settingsService.getSettings().then(function (response) {
              ...
              var gmtIncrement = gmtItem.Name.substring(
                gmtItem.Name.indexOf('GMT') + 3,
                7
              );
              ...
              if (-UTCOffsetHours !== Number(gmtIncrement)) {
                ...
                //UTCDate
                var momentDate = moment(fullDate).add(
                  Number(gmtIncrement),
                  'hours'
                );
                //to gmt local
              ...
              var roundedMinutes = Math.ceil(
                Math.round(moment(fullDate).minute() / 15) * 15
              );
    ...
    function isDateExpired(dateString, hour, minute, idTimezone) {
      var defer = $q.defer();
      getFullDateinLocalTime(dateString, hour, minute, idTimezone).then(
        function (fullDate) {
    ...
    function isTrialExpired() {
      if (
        headerData.user.plan.hasOwnProperty('upgradePending') &&
        headerData.user.plan.upgradePending == false
      ) {
        return false;
      }
      var initialCondition = automation.getModel().initialCondition;
      var condition =
        validInitialConditionByType[initialCondition.type](initialCondition) ||
        false;
    ...
    function getAutomationStartDateByDay(day, time, idTimezone) {
      ...
      var timeZoneOffeset =
        -1 * getTimeZoneOffeset(idTimezone) - startDate.getTimezoneOffset();
      ...
      startDate.setDate(
        startDate.getDate() + (distance === 0 && _isNextWeek ? 7 : distance)
      );
    ...
    function getAutomationStartDateByDate(date, time, idTimezone) {
      ...
      startDate.setMonth(
        _isNextMonth ? startDate.getMonth() + 1 : startDate.getMonth()
      );
      return addTimeZoneOffset(startDate, timeZoneOffeset);
    }

    function getTimeZoneOffeset(idTimeZone) {
      var timezoneArrayFilter = settingData.timeZones.filter(function (tz) {
        return tz.IdUserTimeZone === idTimeZone;
      });
      return timezoneArrayFilter.length === 1
        ? timezoneArrayFilter[0].Offset
        : 0;
    }
    ...
    var validTrialExpirationDateByDate = function (condition) {
      ...
      var timeZoneOffeset =
        -1 * getTimeZoneOffeset(condition.frequency.timezone) -
        startAt.getTimezoneOffset();
      startAt.setHours(condition.frequency.time.hour);
      startAt.setMinutes(condition.frequency.time.minute);
      return (
        addTimeZoneOffset(startAt, timeZoneOffeset) >
        new Date(headerData.user.plan.trialExpirationDate)
      );
    };
    ...
    var validTrialExpirationDateByWeekDay = function (condition) {
      var result = condition.frequency.days.some(function (day) {
        return (
          getAutomationStartDateByDay(
            day,
            condition.frequency.time,
            condition.frequency.timezone
          ) > new Date(headerData.user.plan.trialExpirationDate)
        );
      });
      return result;
    ...
    var validTrialExpirationDateByMonthDay = function (condition) {
      return (
        getAutomationStartDateByDate(
          condition.frequency.day,
          condition.frequency.time,
          condition.frequency.timezone
        ) > new Date(headerData.user.plan.trialExpirationDate)
      );
