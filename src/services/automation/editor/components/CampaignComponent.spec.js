...
describe('CampaignComponent', function() {
  ...
  beforeEach(inject(function(_CampaignComponent_, _COMPONENT_TYPE_, _TEST_OPTION_) {
 ...
 it('should be able to instantiate a new Campaign component with data', function() {
    ...
    var campaignComponentParams = {
        ...
        viewOnlineLink: '"http://actionsDomain.com/OnlineView/PublicOnlineView?idCampaign=1002271"',
    ...
    expect(campaignComponent.thumbnailUrl).toBe(campaignComponentParams.thumbnailUrl);
    expect(campaignComponent.replyEmail).toBe(campaignComponentParams.replyEmail);
    expect(campaignComponent.socialShares.name).toBe(campaignComponentParams.socialShares.name);
    expect(campaignComponent.contentType).toBe(campaignComponentParams.contentType);
    expect(campaignComponent.DMARCAcceptedDomain).toBe(campaignComponentParams.DMARCAcceptedDomain);
    ...
    expect(campaignComponent.viewOnlineLink).toBe(campaignComponentParams.viewOnlineLink);
    expect(campaignComponent.emailOption).toBe(campaignComponentParams.emailOption);
