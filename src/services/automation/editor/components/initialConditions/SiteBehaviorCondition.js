(function() {
  ...
  angular
    ...
    .factory('SiteBehaviorCondition', ['BaseInitialConditionComponent', 'TIMES_BACK_AUTOMATION', 'TIME_UNIT', 'CONDITION_OPERATOR', 'DOMAIN_STATUS', 'MAX_HOURS_VERIFICATION', 'MAX_DAYS_VERIFICATION', 'MAX_MINUTES_VERIFICATION', 'MAX_WEEKS_VERIFICATION', function(BaseInitialConditionComponent, TIMES_BACK_AUTOMATION, TIME_UNIT, CONDITION_OPERATOR, DOMAIN_STATUS, MAX_HOURS_VERIFICATION, MAX_DAYS_VERIFICATION, MAX_MINUTES_VERIFICATION, MAX_WEEKS_VERIFICATION) {
      ...
      SiteBehaviorCondition.prototype = Object.create(BaseInitialConditionComponent.prototype);
      SiteBehaviorCondition.prototype.markup = '<dp-editor-site-behavior-condition component="component"></dp-editor-site-behavior-condition>';
      SiteBehaviorCondition.prototype.panelTemplate = '<div dp-editor-panel-site-behavior-condition class="dp-editor-panel-site-behavior-condition"></div>';
      ...
      SiteBehaviorCondition.prototype.checkCompleted = function() {
        ...
        if (this.domains.length) {
          _.each(this.domains, function(domain){
            if (!domain.url || !domain.url.length || domain.url.indexOf('?') > -1
        ...
        if (this.domains && (this.domains.length > 1 ||
          ...
          isCompleted = isCompleted && isVerificationTimeValid(this.timeUnit, this.verificationTime);
        }
      ...
      function isVerificationTimeValid(timeUnit, verificationTime) {
        return verificationTime >= 0 && Number.isInteger(parseInt(verificationTime))
            && ((timeUnit === TIME_UNIT.HOURS && verificationTime <= MAX_HOURS_VERIFICATION)
            || (timeUnit === TIME_UNIT.DAYS && verificationTime <= MAX_DAYS_VERIFICATION)
            || (timeUnit === TIME_UNIT.MINUTES && verificationTime <= MAX_MINUTES_VERIFICATION)
            || (timeUnit === TIME_UNIT.WEEKS && verificationTime <= MAX_WEEKS_VERIFICATION));
      }
