(function() {
  ...
  function facebookService($http) {
    ...
    function getAuthorization() {
      return $http.get('/Lists/Form/StartFacebookAuthorizationV2').then(function(response) {
        return response.data.config;
      });
    }

    function hasPermissionInFacebook() {
      return $http.get('/Lists/Form/CheckFacebookPermission').then(function(response) {
        return response.data.success && response.data.permission;
      });
    }

    function getFacebookDataModel(idForm) {
      return $http.get('/Lists/Form/GetTabInformation', { params: { idForm: idForm } }).then(function(response) {
    ...
    function saveTab(tabModel, idForm, file, removeImage) {
      ...
      return $http.post('/Lists/Form/SaveTabInformation', formData, {
    ...
    function deleteTab(idForm) {
      return $http.post('/Lists/Form/DeleteTab', {FormId: idForm}).then(function(response) {
    ...
    function uploadImage(file, formId) {
      ...
      return $http.post('/Lists/Form/BaseFacebookTabLogoUpload', formData,
        {
          transformRequest: angular.identity,
          headers: {'Content-Type': undefined}
        }).then(function(response) {
