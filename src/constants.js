...
var Regex = {
  CUSTOMDOMAIN:
    /(?=^.{1,254}$)(^(?:(?!\d+\.)[a-zA-Z0-9_\-ñÑ]{1,63}\.?)+(?:[a-zA-Z]{2,})$)/i,
  DOMAIN: /^((?!www\.)(([a-zA-Z0-9\-_ñÑ]+\.)+)([a-zA-Z]{2,16}))$/i, // eslint-disable-line
  DOMAIN_HTTP:
    /^((http(s)?(:\/\/))?(www\.)?(([a-zA-Z0-9\-_ñÑ]+\.)+)([a-zA-Z]{2,16}))$/i, // eslint-disable-line
  URL_WITH_SUBFOLDERS:
    /^((http(s)?(:\/\/))?(www\.)?(([a-zA-Z0-9\-_]+\.)+)([a-zA-Z]{2,16})(\/+(([a-zA-Z0-9\-_])+))*(\/\*?)?)$/i, // eslint-disable-line
  REGEX_SMS_STRING:
    '@£$¥èéùìòÇ`Øø`ÅåΔ_ΦΓΛΩΠΨΣΘΞ`ÆæßÉ !"#¤%&\'()*=,-./0123456789:;<=>?¡ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÑÜ`¿abcdefghijklmnopqrstuvwxyzäöñüà', // eslint-disable-line
  REGEX_SMS_GSM_EXTENDED_STRING:
    '````````````````````^```````````````````{}`````\\````````````[~]`|````````````````````````````````````€``````````````````````````', // eslint-disable-line
  URL_WITH_SUBFOLDERS_HTTPS_ONLY:
    /^(https:\/\/(www\.)?(([a-zA-Z0-9\-_]+\.)+)([a-zA-Z]{2,23})(\/+(([a-zA-Z0-9\-_.?=&#])+))*(\/\*?)?)$/i, // eslint-disable-line
