  ...
  function AutomationGridCtrl(
    ...
    function loadData() {
      $scope.gridModel.getListData().then(function (response) {
        $scope.totalTasks =
          $scope.totalTasks === 0
            ? response.data.tasksQuantity
            : $scope.totalTasks;
        $scope.isRegistrationCompleted = response.data.IsRegistrationCompleted;
        $scope.isLoading = false;
        $scope.gridLoading = false;
        $scope.replicateAutomationEnabled =
          response.data.ReplicateAutomationEnabled;
      });
    }
    ...
    $scope.deleteRowConfirmed = function (row) {
      $scope.gridModel
        .deleteRow(row, 'IdScheduledTask', { idTask: row.IdScheduledTask })
    ...
    $scope.openReplicationPopup = function (
      idScheduledTask,
      automationTaskType
    ) {
      return modalService
        .showModal({
          templateUrl:
            'angularjs/partials/shared/modalReplicateAutomation.html',
          ...
          modal.close.then(function (closeModalResult) {
            if (closeModalResult.success) {
              $window.location.href =
                '/Automation/EditorConfig?idScheduledTask=' +
                closeModalResult.idScheduledTask +
                '&automationType=' +
                closeModalResult.scheduledTaskType;
            }
          });
        });
    };
    ...
    function updateDisplayedStatus(item) {
      switch (Number(item.StatusId)) {
        case 1:
          item.statusDisplayed = $translate.instant(
            'automation_grid_status.active'
          );
          item.imageText = 'active';
          break;
        case 2:
          item.statusDisplayed = $translate.instant(
            'automation_grid_status.paused'
          );
          item.imageText = 'paused';
          break;
        case 5:
          item.statusDisplayed = $translate.instant(
            'automation_grid_status.draft'
          );
          item.imageText = 'draft';
          break;
        case 6:
          item.statusDisplayed = $translate.instant(
            'automation_grid_status.stopped'
          );
          item.imageText = 'stop';
          break;
        default:
