(function() {
  ...
  function ModalConfigureDkimCtrl($scope, close, data, dkimService, $translate, $timeout) {
    $scope.data = data;
    $scope.dkimSelector = '';
    $scope.dkimPublicKey = '';
    $scope.spfField = '';
    ...
    dkimService.getDkimKeys($scope.data.model.DomainId).then(function(response) {
      if (response.data.success) {
        ...
        $scope.dkimStatusDescription = ($translate.instant('control_panel.dkim.validation_states.description-' + $scope.DkimStatus, { protocol: 'DKIM' })).replace(/<br\/>/g, '');
        $scope.spfStatusDescription = ($translate.instant('control_panel.dkim.validation_states.description-' + $scope.SpfStatus, { protocol: 'SPF' })).replace(/<br\/>/g, '');
      }
    });
    ...
    $scope.verify = function() {
      ...
      dkimService.verifyDomain($scope.data.model.DomainId).then(function(response){
        if (response.data.success) {
          ...
          $scope.row.LastValidationDate = response.data.row.LastValidationDate;
