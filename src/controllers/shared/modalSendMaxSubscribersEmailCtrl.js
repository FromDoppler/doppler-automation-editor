(function() {
  ...
  angular
    .module('dopplerApp')
    .controller('ModalSendMaxSubscribersEmailCtrl', ModalSendMaxSubscribersEmailCtrl);
  ...
  function ModalSendMaxSubscribersEmailCtrl($scope, close, headerService, data) {
    $scope.data = [];
      ...
      angular.forEach($scope.data.QuestionsList, function(value, key) {
        if (value.Answer.AnswerType === 2 || value.Answer.AnswerType === 3) {
          ...
          angular.forEach(value.Answer.List, function(option, index) {
            ...
            if (value.Answer.AnswerType === 3 && index === (value.Answer.AnswerOptions.length - 1)) {
              $scope.data.QuestionsList[key].Answer.Value += $scope.data.QuestionsList[key].Answer.Text + ' - ';
            }
          });
        }
        if (value.Answer.AnswerType === 4) {
    ...
    function activate() {
      headerService.getMaxSubscribersData($scope.domain)
