  ...
  function HeaderCtrl($scope, $rootScope, headerService, ModalService, $translate, taskService, $attrs, $timeout) {
    ...
    function activate() {
      headerService.getHeaderData($attrs.domain).then(function(response){
        ...
        if (!mainMenuData.user.clientManager && !mainMenuData.user.plan.buttonUrl) {
          headerService.getUpgradePlanData(mainMenuData.user.plan.isSubscribers === 'true' ? 4 : 2, mainMenuData.urlBase)
            .then(function (result) {
              var filtered = _.filter(result.ClientTypePlans, function (val) {
                return mainMenuData.user.plan.isSubscribers === 'true' ? val.SubscribersQty > mainMenuData.user.plan.maxSubscribers : val.EmailQty > $scope.data.user.plan.maxSubscribers;
              });
