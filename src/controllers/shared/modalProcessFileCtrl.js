(function() {
  ...
  function ModalProcessFileCtrl($scope, close, $rootScope, importService, $translate, $timeout) {
    ...
    $scope.DRAG_AND_DROP_MESSAGE = '<span class=\'changing-message\' ng-class=\'dragAndDropMsgClass\'>dragAndDropMessage</span>';
    ...
    $scope.updateDragAndDropMessage = function(value) {
      $scope.dragAndDropMessage = $scope.DRAG_AND_DROP_MESSAGE.replace('dragAndDropMessage', value);
    };
    ...
    $scope.updateUploadedFileStatus = function(value, fileName, errorMessage) {
      ...
      case importState.READY:
        ...
        $scope.updateDropButton(true, $translate.instant('modal_process_file.dropButton_ready'));
        $scope.updateDragAndDropMessage($translate.instant('modal_process_file.dropMessage_ready'));
        break;
      // proccessing
      case importState.PROCESSING:
        ...
        $scope.updateDragAndDropMessage($translate.instant('modal_process_file.dropMessage_importing', {
      ...
      case importState.SUCCESS:
        ...
        $scope.dragAndDropMsgClass = '';
        $scope.updateDropButton(true, $translate.instant('modal_process_file.dropButton_imported'));
        $scope.updateDragAndDropMessage($translate.instant('modal_process_file.dropMessage_imported', {
          fileName: $scope.cropFile(fileName, 50)
        }));
        break;
      ...
      case importState.ERROR:
        ...
        $scope.updateDropButton(true, $translate.instant('modal_process_file.dropButton_importedWithError'));
    ...
    $scope.dropzoneConfig = {
      'options': {
        ...
        'dictMaxFilesExceeded': $translate.instant('modal_process_file.errorMessage_maxFiles'),
        'dictFileTooBig': $translate.instant('modal_process_file.errorMessage_fileSize').replace('fileSize', '{{maxFilesize}}'),
        'dictInvalidFileType': $translate.instant('modal_process_file.import_dragAndDrop_InvalidFileType'),
        'forceFallback': false,
      ...
      'eventHandlers': {
        ...
        'addedfile': function(file) {
          ...
          $scope.processing = false;
          $scope.$apply($scope.updateUploadedFileStatus(1));
          $scope.updateDragAndDropMessage($translate.instant('modal_process_file.selected_file', { fileName: $scope.cropFile(file.name, 50) }));
          $rootScope.noFileUploaded = false;
          $scope.updateDropButton(true, $translate.instant('modal_process_file.dropButton_imported'));
          $scope.$digest();
          $rootScope.$digest();
        ...
        'uploadprogress': function(file, progress) {
          ...
          if (progressDec <= 0.9) {
            ...
            $scope.$apply($scope.updateUploadedFileStatus(3, file.name.toString(), '', ''));
          }
        },
