  ...
  function ModalCreateDkimCtrl($scope, close, data, dkimService, $translate) {
    ...
    $scope.addDomain = function () {
      if ($scope.createDomainForm.$valid) {
        if (
          !!_.find($scope.data.domainCurrentList, function (item) {
            // eslint-disable-line no-extra-boolean-cast
            return (
              item.DomainName.toLowerCase() === $scope.DomainName.toLowerCase()
            );
          })
        ) {
          ...
          dkimService
            .validateDuplicateOtherUser($scope.DomainName)
            .then(function (response) {
              if (response.data.success && response.data.exists) {
                $scope.duplicatedOtherUser = true;
                $scope.duplicatedOtherUserMessage = $translate.instant(
                  'control_panel.dkim.add_domain.domain_duplicated_other_user',
