(function () {
  ...
  function FormIntegrateCtrl(
    ...
    $translate.onReady().then(function () {
      vm.integrations = [
        $translate.instant(
          'forms_integrate.headers.integration_landing.description'
        ),
        $translate.instant(
          'forms_integrate.headers.integration_embed.description'
        ),
      ...
      formsService.getIntegrations(urlParams.idForm).then(function (data) {
        if (data.success) {
          ...
          if (vm.type === vm.FORM_TYPE.INLINE) {
            vm.integrations.push(
              $translate.instant(
                'forms_integrate.headers.integration_iframe.description'
              )
            );
            vm.iframeText =
              "<iframe id='doppler_subscription' src='" +
              vm.integrationLink +
              "' height='335' width='375' scrolling='yes' frameborder='no' style='overflow:scroll'></iframe>";
          }
          if (
            vm.type === vm.FORM_TYPE.MODAL ||
            vm.type === vm.FORM_TYPE.WHATSAPP
          ) {
