(function () {
  ...
  function tiendanubeIntegrationCtrl(
    $scope,
    $translate,
    ModalService,
    tiendanubeService,
    ...
    $translate.onReady().then(function () {
      vm.entityPlaceholder = $translate.instant(
        'tiendanube_integration.connected.select_entity_placeholder'
      );
      vm.listPlaceholder = $translate.instant(
        'tiendanube_integration.connected.select_list_placeholder'
      );
      vm.getStatus(true);
      loadDopplerFields();
      loadFieldTypes();
    ...
    vm.getStatus = function (doPolling) {
      return tiendanubeService.getIntegrationStatus().then(function (result) {
        if (result.success) {
          if (!!result.model) {
            // eslint-disable-line
            ...
            vm.allTiendanubeEntitiesList === undefined
              ? vm.getTiendanubeEntitiesList()
          ...
          vm.errorMsg = $translate.instant(
            'tiendanube_integration.disconnected.connection_error'
          );
        }
    ...
    vm.getUserList = function () {
      vm.isLoading = true;
      tiendanubeService.getUserLists().then(function (listResult) {
    ...
    vm.getTiendanubeEntitiesList = function () {
      ...
      tiendanubeService.getTiendanubeEntities().then(function (listResult) {
        vm.importingListDropdown = false;
        if (listResult.success) {
          vm.allTiendanubeEntitiesList = _.map(
            listResult.modules,
            function (entity) {
        ...
        inputs: {
          data: {
            title: $translate.instant(
              'tiendanube_integration.connected.disconnect_popup.title'
            ),
            description: $translate.instant(
              'tiendanube_integration.connected.disconnect_popup.description'
            ),
    ...
    vm.synchronizeAllLists = function () {
      if (!vm.importingAllLists) {
        ...
        tiendanubeService.manualSync().then(function () {
          vm.checkListState();
        });
        $timeout(function () {
    ...
    vm.showMappingSection = function (fieldsMapped) {
      ...
      tiendanubeService
        .getTiendanubeFields(vm.selectedEntity.ApiName)
    ...
    vm.onEntitySelected = function () {
      var listName = $translate.instant(
        'tiendanube_integration.connected.lists.' + vm.selectedEntityId
      );
    ...
    function insertEmailAtTheBeginningIfExists() {
      var tiendanubeFieldsExtracted = _.partition(
        vm.tiendanubeFields,
        function (field) {
      ...
      if (tiendanubeEmailFields.length > 0) {
        ...
        vm.tiendanubeFields = _.union(
          tiendanubeEmailFields,
          tiendanubeFieldsListWithoutEmails
        );
      }
    }
    ...
    function integrateAndSynchronize() {
      vm.isMapping = true;
      tiendanubeService
        .integrateTiendanubeList(vm.selectedList, vm.selectedEntity)
        .then(function (listResult) {
          if (listResult.success) {
    ...
    function mapFieldsAndSynchronize() {
      tiendanubeService
        .associateTiendanubeFieldMapping(
          vm.selectedListId,
    ...
    function validateEmptyEmail() {
      var emailFieldSelected = _.find(
        vm.tiendanubeFields,
        function (tiendanubeField) {
          return (
            tiendanubeField.idDopplerField === BASIC_FIELD.EMAIL &&
            tiendanubeField.ApiName === 'email'
          );
        }
      );
      emailFieldSelected
        ...
        : showGeneralMappingError(
            $translate.instant(
              'tiendanube_integration.mapping.empty_email_error_message'
            )
          );
    ...
    function filterAvailableEntities() {
      vm.tiendanubeEntitiesList = _.filter(
        vm.allTiendanubeEntitiesList,
        function (entity) {
          return (
            _.find(vm.integratedLists, function (integrated) {
          ...
          if (listResult.length) {
            ...
            vm.userFields.unshift({
              ...
              name: $translate.instant(
                'tiendanube_integration.mapping.add_field_option'
              ),
              DataType: 0,
              Value: null,
            ...
            vm.userFields.unshift({
              idField: 0,
              name: $translate.instant(
                'tiendanube_integration.mapping.skip_column_option'
              ),
              DataType: 0,
              Value: null,
            });
    ...
    vm.fieldFilter = function (dopplerFieldId, dopplerFieldTypeId) {
      return function (field) {
        return (
          ...
          ((dopplerFieldTypeId != -1
            ? field.type == dopplerFieldTypeId
            : true) &&
            fieldNotUsed(field.idField))
        );
      };
    };
    ...
    vm.createField = function (index) {
      if (vm.newField.name) {
        ...
        vm.newField.error = $translate.instant(
          'tiendanube_integration.mapping.new_field.required_message'
        );
      }
    };
