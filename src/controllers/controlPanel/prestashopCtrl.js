(function () {
  ...
  function prestashopCtrl(
    ...
    function getStatus() {
      prestashopService.getStatus().then(function (response) {
        if (response.success) {
          ...
          if (!vm.connected) {
            ...
            if (
              response.subscriberListStatus ===
              IMPORTING_STATE_STR.IMPORTING_SUBSCRIBERS
            ) {
    ...
    vm.checkListState = function () {
      vm.stateArray = [];
      vm.integratedLists[0].listStatus =
        IMPORTING_STATE_STR.IMPORTING_SUBSCRIBERS;
      vm.disableSync = vm.importingList = true;
      vm.stateArray.push({
        IdSubscribersList: vm.integratedLists[0].listId,
          ...
          prestashopService
            ...
            .then(function (response) {
              if (response.arePending) {
                ...
                prestashopService
                  .getListData(vm.integratedLists[0].listId)
                  .then(function (response) {
                    vm.integratedLists[0].subscribersCount =
                      response.SubscribersCount;
                    vm.integratedLists[0].listStatus =
                      IMPORTING_STATE_STR.READY;
                  });
              }
            });
        })();
    ...
    vm.connect = function () {
      ...
      prestashopService
        .connect(
          ...
          vm.integrationData.url,
          vm.integrationData.listId
        )
        ...
        inputs: {
          data: {
            title: $translate.instant(
              'prestashop.connected.disconnect_popup.title'
            ),
            description: $translate.instant(
              'prestashop.connected.disconnect_popup.description'
            ),
    ...
    function showErrorMessage(errorCode) {
      vm.errorMsg = $translate.instant(
        'prestashop.disconnected.errors.cannot_connect'
      );
      if (errorCode) {
        switch (errorCode) {
          ...
          case INTEGRATION_ERROR_CODES.PS_NO_PERMISSION:
            vm.errorMsg = $translate.instant(
              'prestashop.disconnected.errors.no_permissions'
            );
            break;
          case INTEGRATION_ERROR_CODES.CANNOT_SYNC_LIST:
            vm.errorMsg = $translate.instant(
              'prestashop.disconnected.errors.no_sync_list'
            );
            break;
          case INTEGRATION_ERROR_CODES.UNAUTHORIZED:
            vm.errorMsg = $translate.instant(
              'prestashop.disconnected.errors.unauthorized'
            );
            break;
          default:
            break;
    ...
    function setSelectedList(listId) {
      ...
      var listName = !!selectedList
        ...
        : vm.defaultListName; //eslint-disable-line
