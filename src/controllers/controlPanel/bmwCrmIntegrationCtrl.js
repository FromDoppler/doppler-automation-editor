(function () {
  ...
  function bmwCrmIntegrationCtrl(
    $scope,
    $translate,
    ModalService,
    bmwCrmService,
    ...
    vm.getStatus = function (doPolling) {
      return bmwCrmService.getIntegrationStatus().then(function (result) {
        ...
        if (result.success) {
          ...
          vm.errorMsg = $translate.instant(
            'bmwcrm_integration.disconnected.connection_error'
          );
        }
        ...
        inputs: {
          data: {
            title: $translate.instant(
              'bmwcrm_integration.connected.disconnect_popup.title'
            ),
            description: $translate.instant(
              'bmwcrm_integration.connected.disconnect_popup.description'
            ),
    ...
    vm.getEntitiesList = function () {
      ...
      bmwCrmService.getEntities().then(function (result) {
        vm.importingListDropdown = false;
        if (result.success) {
    ...
    vm.getUserList = function () {
      vm.isLoading = true;
      bmwCrmService.getUserLists().then(function (listResult) {
    ...
    vm.onEntitySelected = function () {
      if (vm.selectedEntityId) {
        var listName = $translate.instant(
          'bmwcrm_integration.connected.lists.' + vm.selectedEntityId
        );
    ...
    vm.synchronizeAllLists = function () {
      if (!vm.importingAllLists) {
        ...
        bmwCrmService.manualSync().then(function () {
          vm.checkListState();
        });
        $timeout(function () {
          ...
          if (listResult.length) {
            ...
            vm.userFields.unshift({
              ...
              name: $translate.instant(
                'bmwcrm_integration.mapping.add_field_option'
              ),
              DataType: 0,
              Value: null,
            ...
            vm.userFields.unshift({
              idField: 0,
              name: $translate.instant(
                'bmwcrm_integration.mapping.skip_column_option'
              ),
              DataType: 0,
              Value: null,
            });
    ...
    function mapFieldsAndSynchronize() {
      bmwCrmService
        .associateFieldMapping(
          vm.selectedListId,
          _.filter(vm.bmwCrmFields, function (entity) {
    ...
    vm.createField = function (index) {
      if (vm.newField.name) {
        ...
        vm.newField.error = $translate.instant(
          'bmwcrm_integration.mapping.new_field.required_message'
        );
      }
    };
