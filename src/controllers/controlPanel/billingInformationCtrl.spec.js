...
describe('Billing Information', function () {
  ...
  var utils = {
    getIdentificationLengthRange: function () {
      return { min: 4, max: 6 };
    },
  };
  var PAYMENT_METHOD = {
  ...
  beforeEach(function () {
    module(function ($provide) {
      ...
      $provide.value(
        'billingInformationSettings',
        billingInformationServiceMock
      );
  ...
  it('should not be able to change payment method to TRANSFER when origin one is CREDIT CARD', function () {
    ...
    expect(billingInformationCtrl.billingInformationData.idPaymentMethod).toBe(
      PAYMENT_METHOD.CC
    );
  });
  ...
  it('should change countries list correctly', function () {
    ...
    billingInformationCtrl.getStatesByCountry(
      COUNTRIES_WITH_TRANSFER.ARGENTINA
    );
    $rootScope.$apply();
    expect(
      billingInformationCtrl.billingInformationData.statesList[0].name
    ).toBe('Buenos Aires');

    billingInformationCtrl.getStatesByCountry(COUNTRIES_WITH_TRANSFER.MEXICO);
    $rootScope.$apply();
    expect(
      billingInformationCtrl.billingInformationData.statesList[1].name
    ).toBe('Baja California');
  });
  ...
  it('should have bank information data in model', function () {
    ...
    expect(billingInformationCtrl.billingInformationData.bankName).toBe(
      'Banco'
    );
    expect(billingInformationCtrl.billingInformationData.bankAccount).toBe(
      1234
    );
  });
  ...
  it('should not show DNI for cc payment for Mexico or Argentina', function () {
    ...
    expect(
      billingInformationCtrl.showDNI(
        COUNTRIES_WITH_TRANSFER.ARGENTINA,
        PAYMENT_METHOD.CC,
        CONSUMER_TYPE.CF
      )
    ...
    expect(
      billingInformationCtrl.showDNI(
        COUNTRIES_WITH_TRANSFER.MEXICO,
        PAYMENT_METHOD.CC,
        CONSUMER_TYPE.RFC
      )
    ).toBe(false);
