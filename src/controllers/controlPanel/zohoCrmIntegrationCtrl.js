(function () {
  ...
  function zohoCrmIntegrationCtrl(
    $scope,
    $translate,
    ModalService,
    zohoCrmService,
    ...
    $translate.onReady().then(function () {
      vm.entityPlaceholder = $translate.instant(
        'zoho_crm_integration.connected.select_entity_placeholder'
      );
      vm.listPlaceholder = $translate.instant(
        'zoho_crm_integration.connected.select_list_placeholder'
      );
      vm.getStatus(true);
      loadDopplerFields();
      loadFieldTypes();
    ...
    vm.getStatus = function (doPolling) {
      return zohoCrmService.getIntegrationStatus().then(function (result) {
        if (result.success) {
          if (!!result.model) {
            // eslint-disable-line
            ...
            vm.allZohoCrmEntitiesList === undefined
              ? vm.getZohoCrmEntitiesList()
          ...
          vm.errorMsg = $translate.instant(
            'zoho_crm_integration.disconnected.connection_error'
          );
        }
    ...
    vm.getUserList = function () {
      vm.isLoading = true;
      zohoCrmService.getUserLists().then(function (listResult) {
    ...
    vm.getZohoCrmEntitiesList = function () {
      ...
      zohoCrmService.getZohoCrmEntities().then(function (listResult) {
        vm.importingListDropdown = false;
        if (listResult.success) {
          vm.allZohoCrmEntitiesList = _.map(
            listResult.modules,
            function (entity) {
      ...
      if (zohoCrm.form.$valid) {
        ...
        var OpenWindow = window.open(
          StartZohoAuthorization +
            '?clientId=' +
            zohoCrm.integrationData.clientId +
            '&clientSecret=' +
            zohoCrm.integrationData.clientSecret,
          windowName,
          windowSize
        );
        OpenWindow.focus();
        ...
        inputs: {
          data: {
            title: $translate.instant(
              'zoho_crm_integration.connected.disconnect_popup.title'
            ),
            description: $translate.instant(
              'zoho_crm_integration.connected.disconnect_popup.description'
            ),
    ...
    vm.synchronizeAllLists = function () {
      if (!vm.importingAllLists) {
        ...
        zohoCrmService.manualSync().then(function () {
          vm.checkListState();
        });
        $timeout(function () {
    ...
    vm.showMappingSection = function (fieldsMapped) {
      ...
      zohoCrmService
        .getZohoCrmFields(vm.selectedEntity.ApiName)
    ...
    function insertEmailAtTheBeginningIfExists() {
      var zohoCrmFieldsExtracted = _.partition(
        vm.zohoCrmFields,
        function (field) {
      ...
      if (zohoCrmEmailFields.length > 0) {
        ...
        vm.zohoCrmFields = _.union(
          zohoCrmEmailFields,
          zohoCrmFieldsListWithoutEmails
        );
      }
    }
    ...
    function integrateAndSynchronize() {
      vm.isMapping = true;
      zohoCrmService
        .integrateZohoCrmList(vm.selectedList, vm.selectedEntity)
        .then(function (listResult) {
          if (listResult.success) {
            mapFieldsAndSynchronize();
          }
    ...
    function mapFieldsAndSynchronize() {
      zohoCrmService
        .associateZohoCrmFieldMapping(
          vm.selectedListId,
          _.filter(vm.zohoCrmFields, function (entity) {
    ...
    function validateEmptyEmail() {
      ...
      emailFieldSelected
        ...
        : showGeneralMappingError(
            $translate.instant(
              'zoho_crm_integration.mapping.empty_email_error_message'
            )
          );
    ...
    function filterAvailableEntities() {
      vm.zohoCrmEntitiesList = _.filter(
        vm.allZohoCrmEntitiesList,
        function (entity) {
          return (
            _.find(vm.integratedLists, function (integrated) {
          ...
          if (listResult.length) {
            ...
            vm.userFields.unshift({
              ...
              name: $translate.instant(
                'zoho_crm_integration.mapping.add_field_option'
              ),
              DataType: 0,
              Value: null,
            ...
            vm.userFields.unshift({
              idField: 0,
              name: $translate.instant(
                'zoho_crm_integration.mapping.skip_column_option'
              ),
              DataType: 0,
              Value: null,
              DopplerFieldTypeId: -1,
    ...
    vm.createField = function (index) {
      if (vm.newField.name) {
        ...
        vm.newField.error = $translate.instant(
          'zoho_crm_integration.mapping.new_field.required_message'
        );
      }
    };
