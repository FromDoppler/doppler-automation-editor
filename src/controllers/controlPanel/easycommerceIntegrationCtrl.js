(function () {
  ...
  function easycommerceIntegrationCtrl(
    $scope,
    $translate,
    ModalService,
    easycommerceService,
    ...
    $translate.onReady().then(function () {
      vm.entityPlaceholder = $translate.instant(
        'easycommerce_integration.connected.select_entity_placeholder'
      );
      vm.listPlaceholder = $translate.instant(
        'easycommerce_integration.connected.select_list_placeholder'
      );
      vm.getStatus(true);
      loadDopplerFields();
      loadFieldTypes();
    ...
    vm.getStatus = function (doPolling) {
      return easycommerceService.getIntegrationStatus().then(function (result) {
        if (result.success) {
          if (!!result.model) {
            // eslint-disable-line
            ...
            vm.allEasycommerceEntitiesList === undefined
          ...
          vm.errorMsg = $translate.instant(
            'easycommerce_integration.disconnected.connection_error'
          );
        }
    ...
    vm.getUserList = function () {
      vm.isLoading = true;
      easycommerceService.getUserLists().then(function (listResult) {
    ...
    vm.getEasycommerceEntitiesList = function () {
      ...
      easycommerceService.getEasycommerceEntities().then(function (listResult) {
        vm.importingListDropdown = false;
        if (listResult.success) {
          vm.allEasycommerceEntitiesList = _.map(
            listResult.entities,
            function (entity) {
        ...
        inputs: {
          data: {
            title: $translate.instant(
              'easycommerce_integration.connected.disconnect_popup.title'
            ),
            description: $translate.instant(
              'easycommerce_integration.connected.disconnect_popup.description'
            ),
    ...
    vm.synchronizeAllLists = function () {
      if (!vm.importingAllLists) {
        ...
        easycommerceService.manualSync().then(function () {
          vm.checkListState();
        });
        $timeout(function () {
    ...
    vm.onEntitySelected = function () {
      if (vm.selectedEntityId) {
        var listName = $translate.instant(
          'easycommerce_integration.connected.lists.' + vm.selectedEntityId
        );
    ...
    function insertEmailAtTheBeginningIfExists() {
      var easycommerceFieldsExtracted = _.partition(
        vm.easycommerceFields,
        function (field) {
      ...
      if (vtexEmailFields.length > 0) {
        var easycommerceFieldsListWithoutEmails =
          easycommerceFieldsExtracted[1];
        vm.easycommerceFields = _.union(
          vtexEmailFields,
          easycommerceFieldsListWithoutEmails
        );
      }
    }
    ...
    function integrateAndSynchronize() {
      easycommerceService
        .integrateEasycommerceList(vm.selectedList, vm.selectedEntity)
        .then(function (listResult) {
          if (listResult.success) {
    ...
    function mapFieldsAndSynchronize() {
      easycommerceService
        .associateEasycommerceFieldMapping(
          vm.selectedListId,
          _.filter(vm.easycommerceFields, function (entity) {
    ...
    function validateEmptyEmail() {
      ...
      emailFieldSelected
        ...
        : showGeneralMappingError(
            $translate.instant(
              'easycommerce_integration.mapping.empty_email_error_message'
            )
          );
    ...
    function filterAvailableEntities() {
      vm.easycommerceEntitiesList = _.filter(
        vm.allEasycommerceEntitiesList,
        function (entity) {
          return (
            _.find(vm.integratedLists, function (integrated) {
          ...
          if (listResult.length) {
            ...
            vm.userFields.unshift({
              ...
              name: $translate.instant(
                'easycommerce_integration.mapping.add_field_option'
              ),
              DataType: 0,
              Value: null,
            ...
            vm.userFields.unshift({
              idField: 0,
              name: $translate.instant(
                'easycommerce_integration.mapping.skip_column_option'
              ),
              DataType: 0,
              Value: null,
            });
    ...
    vm.createField = function (index) {
      if (vm.newField.name) {
        ...
        vm.newField.error = $translate.instant(
          'easycommerce_integration.mapping.new_field.required_message'
        );
      }
    };
