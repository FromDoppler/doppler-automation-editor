...
(function() {
  ...
  function billingInformationCtrl($scope, $rootScope, $translate, $timeout, utils, billingInformationService,
    PAYMENT_METHOD, CONSUMER_TYPE, COUNTRIES_WITH_TRANSFER, MX_TYPE_OF_PAYMENT, BILLING_SYSTEM) {
    var vm = this;
    ...
    $translate.onReady().then(function() {
      ...
      vm.generalPlaceholder = $translate.instant('billing_information.general_placeholder');
      vm.monthPlaceholder = $translate.instant('billing_information.payment_ways.month');
      vm.yearPlaceholder = $translate.instant('billing_information.payment_ways.year');
    ...
    function activate() {
      billingInformationService.getBillingInformationData()
        .then(function(response) {
          vm.billingInformationData = response.data;
          if (vm.billingInformationData.ccNumber) {
            vm.billingInformationData.ccNumberEncrypted = vm.billingInformationData.ccNumber;
          }
          if (vm.showDNI(vm.billingInformationData.idCountry, vm.billingInformationData.idPaymentMethod,
          ...
          vm.updateIdentificationNumberLength(vm.billingInformationData.ccIdentificationType);
    ...
    vm.getStatesByCountry = function(idCountry) {
      billingInformationService.getStatesByCountry(idCountry)
        .then(function(response) {
    ...
    vm.changePaymentMethod = function(paymentMethod) {
      if (vm.PaymentMethodOrigin === vm.PAYMENT_METHOD.TR ||
        vm.PaymentMethodOrigin !== vm.PAYMENT_METHOD.TR && paymentMethod !== vm.PAYMENT_METHOD.TR) {
    ...
    vm.save = function(billingInformationForm) {
      ...
      if (billingInformationForm.$valid && isExpDateValid) {
        vm.saving = true;
        billingInformationService.saveBillingInformation(vm.billingInformationData)
          .then(function(response) {
            if (response.success) {
          ...
          .catch(function() {
            showErrorMessage();
            vm.errorMsg = $translate.instant('billing_information.credit_card_error');
            vm.saving = false;
          });
      }
    };
    ...
    vm.validateExpDate = function(billingInformationForm) {
      if (vm.billingInformationData.ccExpMonth && vm.billingInformationData.ccExpYear &&
    ...
    vm.updateRegExCC = function(ccNumber) {
      vm.cvvLength = 3;
      var isCcNumberEncrypted = !!vm.billingInformationData.ccNumberEncrypted &&
    ...
    vm.updateIdentificationNumberLength = function(idIdentificationType) {
      vm.identificationNumberLength = utils.getIdentificationLengthRange(idIdentificationType);
    };
    ...
    function showErrorMessage(errorCode) {
      ...
      case 'InvalidCreditCard':
        ...
        break;
      default:
        vm.errorMsg = $translate.instant('validation_messages.generic_error_msg');
        break;
      }
