(function () {
  ...
  function SiteTrackingSettingsCtrl(
    $scope,
    siteTrackingService,
    $translate,
    ModalService,
    ...
    function activate() {
      siteTrackingService.getSiteTrackingPreferences().then(function (data) {
    ...
    vm.enableDisabeSiteTracking = function () {
      siteTrackingService
        .saveEnableDisableSiteTracking(vm.isEnableSiteTracking)
        ...
        inputs: {
          data: {
            title: $translate.instant(
              domain
                ? 'site_tracking.edit_domain.title'
                : 'site_tracking.add_domain.title'
            ),
            description: $translate.instant(
              domain
                ? 'site_tracking.edit_domain.description'
                : 'site_tracking.add_domain.description'
            ),
            descriptionLi1: $translate.instant(
              'site_tracking.add_domain.description_li_1'
            ),
            descriptionLi2: $translate.instant(
              'site_tracking.add_domain.description_li_2'
            ),
            buttonCancelLabel: $translate.instant('actions.cancel'),
            buttonPrimaryLabel: $translate.instant(
              'site_tracking.add_domain.add_label_button'
            ),
            ...
            placeholder: $translate.instant(
              'site_tracking.add_domain.placeholder'
            ),
    ...
    vm.openTrackingCodeModal = function (domain) {
      ModalService.showModal({
        templateUrl:
          'angularjs/partials/controlPanel/siteTrackingCodeModal.html',
    ...
    vm.verifyDomain = function (domain) {
      ...
      siteTrackingService
        .verifyDomain(domain.IdDomain)
        .then(function (domainResponse) {
          if (
            !domainResponse ||
            domainResponse.status === DOMAIN_STATUS.ERROR
          ) {
            ModalService.showModal({
              templateUrl:
                'angularjs/partials/controlPanel/siteTrackingUnverifyModal.html',
              controller: 'ModalYesOrNoCtrl',
              inputs: { data: {} },
            });
    ...
    vm.VerifyPushConfiguration = function (domain) {
      siteTrackingService
        .VerifyPushConfiguration(domain.IdDomain)
        .then(function (domainResponse) {
          var isPushConfigured =
            domainResponse.PushConfigurationStatus ===
            PUSH_CONFIGURATION_STATUS.CONFIGURED;
          domain.statusPush = !isPushConfigured ? 'error' : 'verified';
          domain.errorStatePush = !isPushConfigured
            ? 'error_message_' + domainResponse.PushConfigurationStatus
            : undefined;
          domain.isPendingVerify = false;
        });
    };
