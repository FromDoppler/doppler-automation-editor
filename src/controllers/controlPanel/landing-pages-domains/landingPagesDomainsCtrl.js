(function() {
  ...
  function landingPagesDomainsCtrl($scope, $translate, customDomainService, gridService, ModalService) {
    ...
    $scope.getStatusText = function(row) {
      ...
      case Enums.domainVerificationStatus.CONFIGURATIONERROR:
        return $translate.instant('landing_pages_domains.grid.status.configuration_error');
      case Enums.domainVerificationStatus.VERIFIED:
        return $translate.instant('landing_pages_domains.grid.status.verified');
      case Enums.domainVerificationStatus.NOTCONFIGURED:
        return row.statusDisplayed = $translate.instant('landing_pages_domains.grid.status.unconfigured');
      case Enums.domainVerificationStatus.PENDING:
        return row.statusDisplayed = $translate.instant('landing_pages_domains.grid.status.pending');
      default:
    ...
    $scope.openCreationModal = function() {
      return ModalService.showModal({
        templateUrl: 'angularjs/partials/controlPanel/landing-pages-domains/createCustomDomain.html',
    ...
    $scope.openConfigurationModal = function(row) {
      return ModalService.showModal({
        templateUrl: 'angularjs/partials/controlPanel/landing-pages-domains/configureCustomDomain.html',
          ...
          modal.close
            .then(function(domainData) {
              if (domainData) {
                ...
                if (domainData.IdDomainVerificationStatus !== Enums.domainVerificationStatus.VERIFIED) {
    ...
    $scope.verify = function(row) {
      ...
      customDomainService.verify(row.IdCustomDomain)
        .then(function(response) {
          row.validating = false;
          $scope.processing = false;
          $scope.updateRow(response);
          if (response.IdDomainVerificationStatus !== Enums.domainVerificationStatus.VERIFIED) {
            $scope.openDomainVerificationErrorModal();
          }
        })
        .catch(function() {
    ...
    $scope.openDomainVerificationErrorModal = function() {
      return ModalService.showModal({
        templateUrl: 'angularjs/partials/controlPanel/landing-pages-domains/domainVerificationError.html',
    ...
    $scope.delete = function(row) {
      customDomainService.delete(row.IdCustomDomain)
        .then(function() {
        ...
        .catch(function(response) {
          if (response.data.error === 'CustomDomainAssociatedToAForm') {
            showDeleteErrorMessage($translate.instant('landing_pages_domains.delete.error.form_associated'));
          } else {
            showDeleteErrorMessage($translate.instant('landing_pages_domains.delete.error.default'));
          }
        });
    };
