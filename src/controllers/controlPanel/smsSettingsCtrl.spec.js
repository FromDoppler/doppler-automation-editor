...
describe('SMS settings', function () {
  ...
  it('should not break with formatRFC with other consumer types', function () {
    ...
    $rootScope.$apply();
    smsSettingsCtrl.paymentData.billingInformation.idConsumerType =
      CONSUMER_TYPE.RI;
    smsSettingsCtrl.paymentData.billingInformation.cuit = '27348514143';
    smsSettingsCtrl.formatRFC();

    expect(smsSettingsCtrl.paymentData.billingInformation.cuit).toBe(
      '27348514143'
    );
  });
  ...
  it('should order countries by name correctly in estimate screen', function () {
    ...
    */
    ...
    smsSettingsCtrl.showCalcCreditScreen();
    expect(smsSettingsCtrl.availableCountriesToEstimate[0].name).toBe(
      'Argentina'
    );
  ...
  it('should show the most appropiate sms rounded amount', function () {
    ...
    expect(
      smsSettingsCtrl.getRoundedPrice(
        smsSettingsCtrl.estimateCountriesList[0].price,
        smsSettingsCtrl.estimateCountriesList[0].count
      )
    ).toBe(50);
    expect(
      smsSettingsCtrl.getRoundedPrice(
        smsSettingsCtrl.estimateCountriesList[1].price,
        smsSettingsCtrl.estimateCountriesList[1].count
      )
    ).toBe(50.05);
  });
  ...
  it('should not show cuit or DNI for Mexico with Transfer and consumer type selected', function () {
    ...
    expect(
      smsSettingsCtrl.showDNI(
        smsSettingsCtrl.paymentData.billingInformation.idCountry,
        smsSettingsCtrl.selectedPaymentMethod,
        consumerTypeRFC
      )
    ).toBe(false);
  });
  ...
  it('should not show DNI for CC for all countries', function () {
    ...
    expect(
      smsSettingsCtrl.showDNI(
        ...
        smsSettingsCtrl.PAYMENT_METHOD.TR,
        smsSettingsCtrl.CONSUMER_TYPE.RI
      )
    ).toBe(false);
    expect(
      smsSettingsCtrl.showDNI(
        countryCodes.mexico,
        smsSettingsCtrl.PAYMENT_METHOD.TR,
        smsSettingsCtrl.CONSUMER_TYPE.RFC
      )
    ).toBe(false);
    expect(
      smsSettingsCtrl.showDNI(
        countryCodes.argentina,
        smsSettingsCtrl.PAYMENT_METHOD.CC,
        smsSettingsCtrl.CONSUMER_TYPE.CF
      )
    ).toBe(false);
  });
  ...
  it('should complete map correctly fields into service for Colombia transfer', function () {
    ...
    expect(smsSettingsServiceMock.buySmsCredits).toHaveBeenCalledWith(
      previousPaymentData,
      smsSettingsCtrl.creditToBuy,
      smsSettingsCtrl.PAYMENT_METHOD.TR
    );
  });
});
