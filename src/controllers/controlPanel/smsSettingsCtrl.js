(function () {
  ...
  function smsSettingsCtrl(
    ...
    $translate.onReady().then(function () {
      vm.countryPlaceholder = $translate.instant(
        'sms_settings.calc_credit_screen.country_grid.country_placeholder'
      );
      vm.purchaseButton = $translate.instant(
        'sms_settings.buy_screen.buy_button'
      );
      vm.selectOptionPlaceholder = $translate.instant(
        'sms_settings.buy_screen.credit_card.select_option_placehoder'
      );
      vm.ccExpMonthPlaceholder = $translate.instant(
        'sms_settings.buy_screen.credit_card.month_placeholder'
      );
      vm.ccExpYearPlaceholder = $translate.instant(
        'sms_settings.buy_screen.credit_card.year_placeholder'
      );
      activate();
    });
    ...
    function activate() {
      vm.isLoading = true;
      smsSettingsService.getUserSmsData().then(function (response) {
        if (response.status === 200) {
          ...
          vm.selectedCountries = _.filter(vm.countriesList, function (country) {
            return _.find(
              response.data.selectedCountries,
              function (regionCode) {
    ...
    vm.isAddCountryToEstimateEnable = function () {
      return (
        (!_.find(vm.estimateCountriesList, function (country) {
    ...
    vm.updatePrices = function (regionCode) {
      ...
      _.each(vm.estimateCountriesList, function (countryToEstimate) {
        var countryPrice = countryToEstimate.regionCode
          ? vm.findCountry(countryToEstimate.regionCode).price
          : 0;
    ...
    vm.validDeliveryFrom = function () {
      if (vm.smsDelivery.ShippingSchedulesFrom < 0) {
        ...
        vm.smsDelivery.ShippingSchedulesFrom >
        vm.smsDelivery.ShippingSchedulesTo
      ) {
        vm.smsDelivery.ShippingSchedulesFrom =
          vm.smsDelivery.ShippingSchedulesTo;
      }
    ...
    vm.getDefaultCountryCount = function (price) {
      ...
      return vm.getRoundedPrice(count, price) >= vm.minPurchase
        ? count
        : count + 1;
    };
    ...
    vm.saveChanges = function () {
      if (!vm.saving) {
        vm.saving = true;
        var regionCodes =
          _.map(vm.selectedCountries, function (country) {
            return country.regionCode;
          }) || [];

        var promiseSmsCountries =
          smsSettingsService.saveSelectedSmsCountries(regionCodes);
        var promiseSmsConfig = smsSettingsService.saveDeliverySmsConfig(
          vm.smsDelivery
        );
        Promise.all([promiseSmsCountries, promiseSmsConfig]).then(function (
          responses
        ) {
          $timeout(function () {
            vm.saving = false;
    ...
    vm.setAvailableCountries = function () {
      vm.availableCountries = _.sortBy(
        _.filter(vm.countriesList, function (country) {
    ...
    vm.setAvailableCountriesToEstimate = function () {
      vm.availableCountriesToEstimate = _.sortBy(
        _.filter(vm.countriesList, function (country) {
    ...
    vm.saveUpdatedActiveCountries = function () {
      ...
      var updatedActiveCountries = _.union(
        estimatedRegionCodes,
        selectedRegionCodes
      );
      return smsSettingsService.saveSelectedSmsCountries(
        updatedActiveCountries
      );
    };
    ...
    vm.buy = function (billingInformationForm) {
      if (billingInformationForm.$valid) {
        ...
        smsSettingsService
          .buySmsCredits(
            vm.paymentData.billingInformation,
            vm.creditToBuy,
          ...
          .then(function (response) {
            if (response.success) {
              ...
              vm.saveUpdatedActiveCountries()
                .catch(function (e) {
                  console.log(
                    'Exception occured when updating active countries',
                    e
                  );
                })
                .finally(function () {
    ...
    vm.formatRFC = function () {
      if (
        vm.paymentData.billingInformation.idConsumerType === CONSUMER_TYPE.RFC
      ) {
        vm.paymentData.billingInformation.cuit = utils.formatValidRfc(
          vm.paymentData.billingInformation.cuit
        );
      }
    };
    ...
    vm.showCuit = function (consumerTypeId) {
      return (
        !!consumerTypeId &&
        consumerTypeId !== vm.CONSUMER_TYPE.CF &&
        vm.paymentData.billingInformation.idCountry ===
          vm.COUNTRIES_WITH_TRANSFER.ARGENTINA
      );
    };
    ...
    function showPaymentModal() {
      var templateURL =
        vm.selectedPaymentMethod === PAYMENT_METHOD.TR
