...
print_help () {
    ...
    echo ""
    echo "Examples:"
    echo "  sh clean.sh --port=22 --destination=cdndoppler@reporting.fromdoppler.com:/cdndoppler/doppler-automation-editor-mfe/ --environment=main"
    echo "  sh clean.sh --port=22 --destination=cdndoppler@reporting.fromdoppler.com:/cdndoppler/doppler-automation-editor-mfe/ --environment=pr --keep-days=7"
}
...
extract_manifest_assets () {
  ...
  tr '"[],' '\n' < "${manifest}" \
    | sed 's/^[[:space:]]*//; s/[[:space:]]*$//' \
    | sed -n \
        -e 's#^https\{0,1\}://[^[:space:]]*/\(static/[^"?[:space:]]*\)$#\1#p' \
        -e 's#^\(static/[^"?[:space:]]*\.[^"/?[:space:]]*\)$#\1#p' \
